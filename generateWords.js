...
for (word in allWords) {
    if (allWords[word].length == 5) availableWords.push(allWords[word]);
}
var correctWords = {};
for (let i = 0; i < 365; i++) {
    const word = availableWords[Math.floor(Math.random() * availableWords.length)];
    const date = new Date(Date.now() + i * 1000 * 60 * 60 * 24);
    correctWords[date.getDate() + '-' + date.getMonth() + '-' + date.getFullYear()] = word;
}
fs.writeFileSync('./words.json', JSON.stringify([correctWords, availableWords]));*/
